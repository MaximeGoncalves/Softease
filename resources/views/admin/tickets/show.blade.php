...
@section('content')
    @php setlocale(LC_TIME, 'fr_FR.utf8','fra'); @endphp
    {{--{{setlocale(LC_ALL, 'french')}}--}}
    <div class="container-fluid p-0">
        <div class="col">
            @if (Auth::user()->hasRole('ROLE_ADMIN')||Auth::user()->hasRole('ROLE_TECHNICIAN'))
                {!! Form::open(['route' => ['ticket.update', $ticket->id],'method' => 'put']) !!}
                <div class="row mb-2">
                    <div class="col-sm-4">
...
{{--                        <input name="appointment" id="datetimepicker" class="form-control" value="{{ $ticket->appointment ? strftime('Intervention prévu : %A %d %B, %Hh%M ',strtotime($ticket->appointment)) : '' }}"/>--}}
                        <input name="appointment" id="datetimepicker" class="form-control" value="{{ $ticket->appointment}}"/>
                    </div>
                </div>
            </div>
                {{Form::close()}}
